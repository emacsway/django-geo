...
class CategoryModelForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        if 'instance' in kwargs.keys():
            ...
            a2 = descendants.values('pk')
            a = []
            for v in a2:
                a.append(v['pk'])
            #import rpdb2; rpdb2.start_embedded_debugger('111')
            qs = Category.objects.exclude(Q(pk__in=a) | Q(pk=instance.pk))
            self.fields['parent_field'].queryset = qs
            
            if 'parent_field' not in self.initial:
                self.initial['parent_field'] = instance.get_parent().pk
                #object_data.update(self.data)
